...
function Dark() {
    ...
    return (
      <> 
    <div className=" text-dBrand dark:text-mainBg hover:left-[-2px] hover:opacity-100 transform duration-500 opacity-50 fixed flex flex-col z-50 max-md:bottom-10 md:top-1/3 left-[-14px] bg-secondaryBg dark:bg-balBrand border-dBrand dark:border-secondaryBg border-2 dark:border-0 rounded-[999px]">
      ...
      <button onClick={()=>{setDarkMode(!darkMode)}} className="flex justify-center cursor-pointer relative w-6 h-11 dark:bg-dBrand bg-mainBg overflow-hidden border-dBrand dark:border-mainBg  border-[1px] rounded-[999px]" aria-label="Dark Mode Switch">
        <div className={`absolute w-5 h-5 dark:bg-mainBg border-[1px] duration-300 border-dBrand dark:border-mainBg rounded-[999px] ${darkMode ? 'translate-y-full' : ''} bg-lBrand`}></div>
      </button>
      <div className="flex flex-row justify-center items-center ">
